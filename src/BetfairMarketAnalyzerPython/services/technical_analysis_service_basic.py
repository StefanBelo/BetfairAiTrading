...
class TechnicalAnalysisServiceBasic:
    """
    Basic technical analysis service using only pandas and numpy
    ...
    def _calculate_rsi(self, prices: np.ndarray, period: int = 14) -> np.ndarray:
        """
        Calculate RSI using basic numpy operations
        """
    ...
    def _calculate_macd(self, prices: np.ndarray, fast: int = 12, slow: int = 26, signal: int = 9) -> Tuple[np.ndarray, np.ndarray]:
        ...
        signal_line = self._calculate_ema(macd[~np.isnan(macd)], signal)
        ...
        if signal_start < len(prices):
            macd_signal[signal_start:signal_start + len(signal_line)] = signal_line
        
        return macd, macd_signal
    
    def _calculate_technical_indicators(self, df: pd.DataFrame) -> TechnicalIndicators:
        """
        Calculate technical indicators using basic numpy operations
        """
        if len(df) < 14:
        ...
        try:
            ...
            sma10 = self._calculate_sma(close_prices, 10)
            ema5 = self._calculate_ema(close_prices, 5)
            ema10 = self._calculate_ema(close_prices, 10)
