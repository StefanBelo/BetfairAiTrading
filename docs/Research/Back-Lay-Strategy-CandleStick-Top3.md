...
Identify the best back or lay bet for the first, second, or third favourite in any Betfair market, using candlestick price history and volume analysis. Analyze correlation in candlestick price movements between the three favourites to improve prediction. The strategy executed is a simple back or lay bet of 10 Euro on the best candidate, indicating if the horse wins or loses.
...
```
On active market, for the three favourite selections, use GetDataContextForMarketSelection with data context "MarketSelectionsCandleStickData" for all three. Analyze candlestick price and volume history for each, and evaluate:
...
- Correlation in candlestick price movements between the three favourites (e.g., do odds move together or inversely?)
...
Use correlation analysis to better predict price movement for the best of the three favourites. Recommend the best trade direction (back or lay), entry odds, and indicate if the horse is expected to win or lose.

All analysis and execution must be performed automatically for the best of the three favourite selections, without asking for confirmation or summary. Automatically execute the recommended strategy using the appropriate MCP call, without requiring operator confirmation or manual intervention.
```
...
- `GetDataContextForMarketSelection` with `dataContextNames: ["MarketSelectionsCandleStickData"]` — retrieve candlestick price history and volume for the three favourite selections.
...
| Selection | Odds | Volume | Direction | Entry | Suggestion | Note | Correlation | Win/Lose |
|---|---:|---:|---|---:|---|---|---|---|
...
- `Correlation` — brief summary of candlestick price movement relationship between the three favourites (e.g., positive, negative, none)
...
The execution must be performed automatically for the best of the three favourite selections and direction, without requiring operator confirmation or manual intervention.
