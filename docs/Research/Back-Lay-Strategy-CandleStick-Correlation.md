...
Identify the best trade (back or lay) for the first and second favourite selections in any Betfair market, using candlestick price history and volume analysis. Analyze correlation in candlestick price movements between the two favourites to improve prediction of price movement for the first favourite. Apply stock market candlestick analysis and correlation techniques if applicable.
...
```
On active market, for the two favourite selections, use GetDataContextForMarketSelection with data context "MarketSelectionsCandleStickData" for both. Analyze candlestick price and volume history for each, and evaluate:
...
- Correlation in candlestick price movements between the two favourites (e.g., do odds move together or inversely?)
...
- `GetDataContextForMarketSelection` with `dataContextNames: ["MarketSelectionsCandleStickData"]` — retrieve candlestick price history and volume for both favourite selections.
...
- `Correlation` — brief summary of candlestick price movement relationship between the two favourites (e.g., positive, negative, none)
...
**Important:**
- The strategy should set `OpenBetPosition.Odds` only when the analysis identifies a more optimal entry odds than the current market offer (e.g., based on support/resistance, volatility, or candlestick signals). If no better entry is found, omit this parameter and the strategy will use the current offered odds by default.
