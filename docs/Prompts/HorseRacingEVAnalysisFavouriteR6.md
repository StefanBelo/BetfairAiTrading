...
- **Minimum EV for LAY**: -8% (Conservative laying threshold)
- **Maximum EV for LAY**: -50% (Upper limit to avoid extreme negative scenarios)

**NO ACTION ZONE:**
- **EV between -8% and +8%**: No strategy executed (insufficient edge)
- **EV beyond ±50%**: No strategy executed (potentially unreliable calculations)
...
Else If Adjusted EV <= -8% AND Adjusted EV >= -50%:
    Consider LAY strategy (subject to additional criteria)
Else:
...
- **EV is beyond ±50%** (potentially unreliable calculation)
...
- Higher EV threshold for laying: -10% (less likely to lay in weak fields)

**MODERATE FIELD Strategy:**
- Standard EV thresholds: ±8%
- Balanced approach

**STRONG FIELD Strategy:**
- Higher EV threshold for backing: +10% (tougher competition)
- Lower EV threshold for laying: -6% (more opportunities to lay)
...
- Report: "LAY executed - EV: [X]% (within -50% to -8% range)"

**If NO ACTION:**
Do not execute any strategy and report:
- "NO ACTION - EV: [X]% (outside actionable range of ±8% to ±50%)" OR
...
- **Conservative Range (±8%)**: Reduces false positives and overconfident betting
- **Maximum Range (±50%)**: Prevents action on potentially erroneous calculations
