---
title: "Horse Racing Expected Value Analysis (Favourite R4) - Timeform & Racing Post Combined Edition"
aliases: ["Horse Racing Expected Value Analysis (Favourite R4) - Timeform & Racing Post Combined Edition"]
type: research
tags: [automation, bfexplorer, ev-analysis, horse-racing, market-sentiment, research]
...
With the `marketId` from the previous step, use the `GetAllDataContextForMarket` tool with `dataContextNames` set to `['TimeformDataForHorses', 'RacingpostDataForHorses']`. This will return comprehensive analysis for each horse combining:
